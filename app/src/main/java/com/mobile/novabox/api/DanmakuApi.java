...
import com.mobile.novabox.util.js.Trans;
...
public class DanmakuApi {
    ...
    /**
     * 云搜：返回候选弹幕列表供用户选择，而不是像 search() 那样自动挑一个最匹配的。
     */
    ...
    /**
     * 加载云搜结果里用户选中的那一条。builtIn 结果还要再请求一次评论接口并转成 xml，
     * 非 builtIn 的结果本身就是可用的弹幕地址，直接回传即可。
     */
    ...
    private static void loadBuiltinComment(String baseUrl, String title, String episode, EpisodeMatch episodeMatch, SearchCallback callback, int seq) {
        loadBuiltinComment(baseUrl, title, episode, episodeMatch, callback, seq, 0);
    }

    private static void loadBuiltinComment(String baseUrl, String title, String episode, EpisodeMatch episodeMatch, SearchCallback callback, int seq, int retry) {
        ...
        OkHttp.newCall(OkHttp.client(BUILTIN_TIMEOUT), commentUrl, TAG).enqueue(new Callback() {
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                if (!isCurrentSearch(seq)) return;
                if (retry < BUILTIN_MAX_RETRY) {
                    LOG.e("echo-danmaku builtin comment error: " + e.getMessage() + ", retry later");
                    handler.postDelayed(() -> {
                        if (isCurrentSearch(seq)) loadBuiltinComment(baseUrl, title, episode, episodeMatch, callback, seq, retry + 1);
                    }, 1500L * (retry + 1));
                } else {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) {
                ...
                try {
                    if (!response.isSuccessful()) {
                        // HTTP 层非 2xx(如 429/5xx)同样按可重试错误处理,复用同一条重试路径
                    ...
                    if (retry < BUILTIN_MAX_RETRY) {
                        LOG.e("echo-danmaku builtin comment parse error: " + th.getMessage() + ", retry later");
                        handler.postDelayed(() -> {
                            if (isCurrentSearch(seq)) loadBuiltinComment(baseUrl, title, episode, episodeMatch, callback, seq, retry + 1);
                        }, 1500L * (retry + 1));
                    } else {
    ...
    private static void notifyNotFound(SearchCallback callback, int seq) {
        ...
        handler.post(() -> { if (isCurrentSearch(seq)) callback.onNotFound(); });
    }
    ...
    private static EpisodeMatch findEpisode(String body, String episode, boolean allowMovieFallback) throws Exception {
        ...
        for (int i = 0; i < episodes.length(); i++) {
            ...
            if (!TextUtils.isEmpty(episode) && !TextUtils.isEmpty(title) && title.contains(episode)) return match;
    ...
    private static boolean isMovieTypeText(String type) { return "\u7535\u5f71".equals(type); }
    ...
    private static boolean prefersCantonese(String episode) { return isCantoneseTitle(episode); }
    ...
    private static class EpisodeList {
        ...
        EpisodeList(JSONArray episodes, boolean isMovie) { this.episodes = episodes; this.isMovie = isMovie; }
    }
    ...
    private static class EpisodeMatch {
        ...
        final String title;
        final int number;
        EpisodeMatch(String id, String title, int number) { this.id = id; this.title = title; this.number = number; }
    }
