...
public abstract class BaseActivity extends AppCompatActivity implements CustomAdapt {
    ...
    /**
     * LoadSir 会把传入 view 从原父容器中取出，替换成自己生成的包装容器（并继承原 view 的
     * LayoutParams，包括 layout_weight），原 view 只是作为包装容器内部的一个 child。
     * 因此仅仅 view.setVisibility(GONE) 并不能让包装容器让出布局空间——包装容器本身
     * 仍然可见，会按原 weight 占位，导致页面出现"空白占位区"。
     * 需要显式控制包装容器（getLoadLayout()）的可见性，才能让兄弟 View 正确接管这部分空间。
     */
    protected void setLoadSirWrapperVisible(boolean visible) {
        if (mLoadService != null && mLoadService.getLoadLayout() != null) {
            mLoadService.getLoadLayout().setVisibility(visible ? View.VISIBLE : View.GONE);
        }
    }
