...
public class ConfigManagerActivity extends BaseActivity {
    ...
    /**
     * 若当前尚未应用任何点播线路（API_URL 为空），
     * 则自动应用列表中最早添加的仓库/线路的第一条线路。
     */
    private void autoApplyFirstRouteIfNone() {
        String current = Hawk.get(HawkConfig.API_URL, "");
        if (!TextUtils.isEmpty(current)) {
            return; // 已有应用中的线路，不做改动
        }
        ...
        List<String[]> routes = getRoutes(firstEntry);
        if (routes.isEmpty()) {
            return;
        }
        String firstRouteUrl = routes.get(0)[1];
        if (TextUtils.isEmpty(firstRouteUrl)) {
            return;
        }
        Hawk.put(HawkConfig.API_URL, firstRouteUrl);
        HistoryHelper.setApiHistory(firstRouteUrl);
        Toast.makeText(this, "已自动应用线路：" + getEntryName(firstEntry), Toast.LENGTH_SHORT).show();
    }
