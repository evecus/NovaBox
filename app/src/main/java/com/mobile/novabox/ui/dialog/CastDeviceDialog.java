...
import com.mobile.novabox.dlna.CastVideo;
import com.mobile.novabox.dlna.DLNACastManager;
...
public class CastDeviceDialog extends BaseDialog {
    ...
    // 本次投屏的地址是否依赖本机代理(127.0.0.1:9978 转换来的局域网地址)。
    // 依赖代理时,投屏成功后需要启动 CastProxyService 前台服务保活,
    // 避免 App 切后台/退出后代理消失导致电视端播放中断。
    private boolean usesLocalProxy;
    // 最近一次发起投屏的目标设备,成功后传给 CastProxyService 用于状态轮询。
    private CastDevice lastCastDevice;
    ...
    /** 标记本次投屏地址是否依赖本机代理(见 CastUrlResolver#resolveWithProxyFlag)。 */
    public void setUsesLocalProxy(boolean usesLocalProxy) {
    ...
    private void handleCastResult(boolean success, String msg) {
        if (success) {
            ...
            if (usesLocalProxy) {
                // 该地址是本机代理转换来的局域网地址,电视端持续播放依赖这台设备
                // 上的 RemoteServer 存活,启动前台服务保活并常驻通知提示用户。
                // 先申请一次通知权限(Android 13+需要),不阻塞后续流程 —— 即使
                // 用户拒绝,CastProxyService 依然会正常启动,只是看不到通知提示。
                // 注意:必须在 dismiss() 关闭对话框之前发起权限请求,并且让系统
                // 弹窗有机会真正显示出来 —— 紧接着立刻 dismiss 弹窗在部分机型上
                // 会导致刚弹出的系统权限对话框被一并带走或一闪而过,用户看不清、
                // 系统就记一次"未授权",之后可能不再弹出申请框。
                final Context appContext = getContext().getApplicationContext();
                final String castTitle = video.getName();
                final CastDevice targetDevice = lastCastDevice;
                com.mobile.novabox.cast.NotificationPermissionHelper.requestIfNeeded(getContext());
                Toast.makeText(getContext(), "该片源依赖本机代理,请勿彻底退出App,可正常切换到后台", Toast.LENGTH_LONG).show();
                if (onCastListener != null) onCastListener.onCastSuccess();
                handler.postDelayed(() -> {
                    // 同时把目标设备传进去:DLNA 设备支持标准状态查询,保活服务会
                    // 定期轮询判断投屏是否已在电视端结束,自动收尾;TVBox 类设备没有
                    // 标准查询接口,服务只能依赖硬性最大保活时长兜底。
                    com.mobile.novabox.cast.CastProxyService.start(appContext, castTitle, targetDevice);
                    dismiss();
                }, 300);
                return;
            }
