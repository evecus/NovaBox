...
//        LOG.i("echo-fixAdM3u8 result: " + result );
    ...
    private static String resolveUriLine(String base, String line) {
        ...
        return value == null ? line : line.replace(value, resolveUri(base, value));
    }

    private static String resolveUri(String base, String url) {
        ...
        try {
            ...
            try {
                ...
                if (url.startsWith("/")) {
                    ...
                    int hostEnd = schemeEnd >= 0 ? base.indexOf('/', schemeEnd + 3) : -1;
                    if (hostEnd >= 0) return base.substring(0, hostEnd) + url;
                    return base + url;
                }
                if (slash >= 0) return base.substring(0, slash + 1) + url;
                return base + "/" + url;
            } catch (Throwable th2) {
                return url;
            }
