...
import xyz.doikki.videoplayer.mpv.MpvPlayerFactory;
...
public class VideoViewConfig {
    ...
    private VideoViewConfig(Builder builder) {
        ...
        if (builder.mPlayerFactory == null) {
            //默认为MPV
            mPlayerFactory = MpvPlayerFactory.create();
        } else {
            mPlayerFactory = builder.mPlayerFactory;
        }
        if (builder.mRenderViewFactory == null) {
            //默认使用GLSurfaceView渲染视频
            mRenderViewFactory = MpvRenderViewFactory.create();
        } else {
